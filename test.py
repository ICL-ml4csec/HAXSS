...
def test_haxss():
        ...
        parser.add_argument('--url', help='url/webapp to fuzz', type=str)
        parser.add_argument('--processes', help='number of processes to use, default = 1', type=int, default=1)
        parser.add_argument('--gamma', help='discount factor, default = 0.99', type=float, default=0.99)
        parser.add_argument('--epsilon', help='probability of random action, default = 0.05', type=float, default=0.05)
        ...
        parser.add_argument('--login', help='login details of the format: username:password', type=str)
        parser.add_argument('--crawl', help='search the webapp before injection, default = True', type=str, default=True)
        parser.add_argument('--max-episodes', help='max number of episodes before fuzzing different source-sink, default = 10', type=int, default=10)
        parser.add_argument('--bypass-episode-len', help='episode length for secondary agent, default = 5 ', type=int, default=5)
        ...
        parser.add_argument('--min-xss', help='min number of xss to find in the range of --xss_range, default = 1', type=int, default=1)
        parser.add_argument('--xss-range', help='number of prior episodes to find the required number of xss in min_xss, default = 1', type=int, default=1)
        args = parser.parse_args()
        site = args.url

        if re.search('https?://.*(/|\.\w+)', site, re.I) is None:
            print('Please input URLs in the following format:')
            print('http://example.com/')
            print('https://second-example.com/path/to/file.php')
            exit(0)
        start_time = time.time()

        if args.crawl is None or (args.crawl and re.search('true', args.crawl, re.I)):
            ...
            print('Loading from saved infomation...')
        ...
        if login_file.exists() and login_file.is_file():
            ...
            login_details = {}
        time_after_crawling = time.time() - start_time
        run_policy(domain=site, processes=args.processes , urls=urls, input_tags=input_tags, transition_tags=transition_tags,
                   ...
                   train=False)
        time_after_testing = time.time() - time_after_crawling - start_time

        print('finished, times:')
        print('crawl time: '+ str(time_after_crawling))
        print('testing time: '+ str(time_after_testing))
        print('total time: '+ str(time_after_crawling + time_after_testing))
