...
class Connections:
	...
	def find_form_method(self, injection_form):
		...
		if fuzz_source.status_code == 404:
			...
			while fuzz_source.status_code == 404 and self.browser_session.current_url != self.source:
	...
	def _get_dependant_form_info(self, url, page_response, update_filler=False, source_idx=None, sink_idx=None):
		...
		for key in payload:
			if key != self.injection_form and not payload[key]:
				if re.search('email', key, re.IGNORECASE) is not None:
					...
					payload[key] = filler
		#if any(re.search('email|username', tag, re.IGNORECASE) is not None for tag in payload):
		...
		if not action:
			...
			for url_part in url.split('/')[:-1]:
				if url_part == 'http:':
					url_part += '/'
				if url_part not in action:
					new_action += url_part + '/'
			#action = url +'/' +action if action[0] != '/' else url + action[1:]
			if new_action not in action:
				action = new_action + action if action[0] != '/' else new_action + action[1:]
		return action, payload
	...
	def login(self, login_usrname, login_details):
		...
		for name in tag_names:
			if re.search('password|pwd', name, re.IGNORECASE):
				...
				try:
					...
					self.browser_session.find_element_by_name(name).click()
					ActionChains(self.browser_session).send_keys(login_usrname)
		...
		if len(self.browser_session.find_elements_by_xpath("//input[@type='password']/ancestor::form")) > 0:
			...
			#self.browser_session.find_elements_by_xpath('//input[@type="password"]/ancestor::div')[-3].find_element_by_xpath(
			#	'.//descendant::*[@type="submit"]').click()
			try:
				self.browser_session.find_elements_by_xpath("//input[@type='password']/ancestor::div")[
			...
			self.browser_session.find_elements_by_xpath('//input[@type="password"]/ancestor::*')[-3].find_element_by_xpath(
				'.//descendant::*[@type="submit"]').click()
		if self.domain not in login_url:
			login_url = self.domain + login_url if login_url[0] != '/' else self.domain + login_url[1:]
	...
	def logout(self):
		...
		get_logout_ttl = 3
		ttl = 0
		while len(logout_link) == 0 and ttl < get_logout_ttl:
			payload_tree = ''
